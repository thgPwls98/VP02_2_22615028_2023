...
namespace _011.EIS
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void btnInsert_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                dateExit = dpEnter.SelectedDate.Value.Date.ToShortDateString();
            }
            else
            {
            ...
            try
            {
                string spl = string.Format("INSERT INTO eis_table(name, department, position, gender, date_enter, date_exit, contact, comment)"
                + "VALUES('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}', '{7}')",
                txtName.Text, dept, pos, gender, dateEnter, dateExit, txtContact.Text, txtComment.Text);

                MySqlCommand cmd = new MySqlCommand(spl, conn);
        ...
        private void dataGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)  //sender=datagrid
        {
            ...
            dpEnter.Text = rowView.Row[6].ToString();
        ...
        private void btnUpdate_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                string sql = string.Format(
                    "UPDATE eis_table SET name='{0}', department='{1}', position='{2}', gender='{3}', date_enter='{4}', date_exit='{5}',contact='{6}',comment='{7}'" +
                    "WHERE eid={8}", txtName.Text, dept, pos, gender, dateEnter, dateExit, txtContact.Text, txtComment.Text, txtEid.Text); //eid는 int이기 떄문에 ''가 필요없음
                MessageBox.Show(sql);
